...
data_stan_panels <- data_complete_cases %>% 
    ...
    select(id_personyear_n, id_n, t_age, lim_adl, lim_iadl,
           ...
           living_spouse, t_female)

##### 4レベルのモデルに渡すリストを作成 ===================================
list_stan_3levels <- c(list(I = nrow(data_stan_individuals)),
...
save(list_stan_3levels, file="~/Data/JAHEAD/Process_Files/data_after_75a_76a.rda")
rm(list = ls())