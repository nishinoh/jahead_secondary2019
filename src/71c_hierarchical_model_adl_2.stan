data {
    ...
    int<lower=0> num_hh_memer[P]; //世帯の人数
    int<lower=0, upper=1> use_dayservice[P]; //特に着目する説明変数
...
transformed parameters {
    ...
    real x_p[P];
    for(p in 1:P)
        x_p[p] = bp_1*t_age[p] + bp_2*t_female[p] + bp_3*lim_adl[p] + bp_4*num_hh_memer[p] + bp_5*use_dayservice[p] + b_p[p];
    for(c in 1:C)
        q[c] = inv_logit(b_0 + b_1*ch_female[c] + b_2*ch_age[c] + b_3*ch_working[c] + b_4*ch_dist_living_l[c] + x_p[id_personyear[c]]);
}
