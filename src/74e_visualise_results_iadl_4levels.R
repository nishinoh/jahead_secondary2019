...
load("~/Data/JAHEAD/Process_Files/stan_result_varying_intercept_iadl_4levels.rda")

post <- summary(fit_model_iadl)$summary %>% 
    #head(., 30) %>% 
    as.data.frame(.) %>% 
...
term_full <- c("実の子ども", "子の性別(女性=1)", "居住場所の距離", "親のIADL困難度", "親が配偶者と居住", "ホームヘルプ利用回数", "親の性別")
...
quartz(file="~/downloads/fig_result_iadl_4levels.pdf", type="pdf",  width=8,height=4.5)
p
dev.off()